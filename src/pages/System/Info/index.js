      ...
      <PageHeaderWrapper>
        <Spin spinning={Boolean(loading)}>
            ...
            title={
              <div>
                ...
                <Tooltip placement="right" title="点击刷新">
                  <a>
                    <Icon type="sync" spin={Boolean(loading)} onClick={this.fetchSystemInfo} />
                  </a>
                </Tooltip>
              </div>
            }
          >
            ...
            <Descriptions.Item label="最近15m平均负载">
              {loadavg && loadavg[2].toFixed(3)}
            </Descriptions.Item>
            <Descriptions.Item label="最近5m平均负载">
              {loadavg && loadavg[1].toFixed(3)}
            </Descriptions.Item>
            <Descriptions.Item label="最近1m平均负载">
              {loadavg && loadavg[0].toFixed(3)}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="CPU核心数">
              <a
                onClick={() =>
                  this.toogolDrawerVisible({ visible: true, title: 'CPU信息', data: cpus })
                }
              >
                {cpus && cpus.length}
              </a>
          ...
          <DetailDrawer
            ...
            title={title}
            data={data}
            toogolDrawerVisible={this.toogolDrawerVisible}
          />
        </Spin>
      </PageHeaderWrapper>
    );
